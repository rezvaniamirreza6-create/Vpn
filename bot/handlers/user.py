    ...
    async with AsyncSessionLocal() as db:
        ...
        if not user:
            user, _ = await crud.get_or_create_user(db, cb.from_user.id,
                username=cb.from_user.username, full_name=cb.from_user.full_name)
    if not plan:
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not user:
            user, _ = await crud.get_or_create_user(db, msg.from_user.id,
                username=msg.from_user.username, full_name=msg.from_user.full_name)
...
async def my_services(event):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not user:
            user, _ = await crud.get_or_create_user(db, uid,
